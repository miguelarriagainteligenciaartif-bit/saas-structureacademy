...
class GlobalErrorBoundary extends React.Component<{children: React.ReactNode}, {hasError: boolean, error: any}> {
  ...
  render() {
    if (this.state.hasError) return <div style={{padding: '2rem', color: 'red'}}><h1>GLOBAL CRASH</h1><pre>{this.state.error?.toString()}</pre><pre>{this.state.error?.stack}</pre></div>;
